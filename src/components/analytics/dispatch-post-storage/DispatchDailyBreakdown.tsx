...
import {
  useGetDispatchDailyBreakdown,
  type GetDispatchDailyBreakdownParams,
} from '@/services/store-admin/dispatch-post-storage/analytics/useGetDispatchDailyBreakdown';
import type { GetDispatchSummaryParams } from '@/services/store-admin/dispatch-post-storage/analytics/useGetDispatchSummary';
import type {
  DispatchTrendData,
  MonthlyTrendChartItem,
} from '@/types/analytics';

interface DispatchDailyBreakdownProps {
...
const DispatchDailyBreakdown = ({
  dateParams,
}: DispatchDailyBreakdownProps) => {
  const [tab, setTab] = useState<TrendTab>('daily');
  const queryParams: GetDispatchDailyBreakdownParams =
    dateParams.dateFrom && dateParams.dateTo
      ? { dateFrom: dateParams.dateFrom, dateTo: dateParams.dateTo }
      : {};

  const dispatchDailyBreakdownQuery = useGetDispatchDailyBreakdown(queryParams);
  const trendData = (dispatchDailyBreakdownQuery.data ??
    {}) as Partial<DispatchTrendData>;
  ...
  if (
    dispatchDailyBreakdownQuery.isLoading ||
    dispatchDailyBreakdownQuery.isFetching
  ) {
    return (
      <Card className="font-custom">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-base font-semibold sm:text-lg">
            ...
            Dispatch Daily Breakdown
          </CardTitle>
          <CardDescription>
            Bags dispatched after storage over time (daily and monthly)
          </CardDescription>
        </CardHeader>
  ...
  if (dispatchDailyBreakdownQuery.isError) {
    return (
      <Card className="font-custom border-destructive/30 bg-destructive/5">
        <CardHeader>
          <CardTitle className="text-destructive">
            Failed to load dispatch daily breakdown
          </CardTitle>
          <CardDescription>
            {dispatchDailyBreakdownQuery.error instanceof Error
              ? dispatchDailyBreakdownQuery.error.message
              : 'Something went wrong.'}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Button
            onClick={() => dispatchDailyBreakdownQuery.refetch()}
            className="gap-2"
          >
...
export default DispatchDailyBreakdown;
