...
export function buildAppliedColumnOrder(
  draftOrder: string[],
  allLeafColumnIds: string[]
): string[] {
  const ordered = draftOrder.filter((id) => allLeafColumnIds.includes(id));
  const missing = allLeafColumnIds.filter((id) => !ordered.includes(id));
