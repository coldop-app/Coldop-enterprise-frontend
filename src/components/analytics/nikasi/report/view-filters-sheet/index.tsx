...
const columnLabels: Record<string, string> = {
  ...
  location: 'Dispatch ledger',
  dispatchLedgerMobile: 'Ledger mobile',
  ...
  isInternalTransferLabel: 'Internal transfer',
      ...
      <div className="space-y-6 p-5">
        <div>
          ...
          <p className="text-muted-foreground mb-3 text-xs">
            Combine filters with AND / OR logic. E.g. internal transfer is Yes
            AND bags issued &gt; 10.
          </p>
          <LogicBuilder
...
export function ViewFiltersSheet({
  ...
  const [draftInternalTransferFilters, setDraftInternalTransferFilters] =
    React.useState<InternalTransferFilterValue[]>(
      internalTransferFilterOptions
    );
    ...
    () => [
      {
        ...
        label: 'Filters',
        description:
          'Refine rows by internal transfer, column values, and logic.',
  ...
  const syncDraftFromTable = React.useCallback(() => {
    ...
    const rawInternalFilter = table
      .getColumn('isInternalTransferLabel')
      ?.getFilterValue();

    setDraftInternalTransferFilters(
      Array.isArray(rawInternalFilter)
        ? (rawInternalFilter as InternalTransferFilterValue[])
        : [...internalTransferFilterOptions]
    );
  ...
  const handleApplyView = React.useCallback(() => {
    ...
    internalTransferColumn?.setFilterValue(
      draftInternalTransferFilters.length ===
        internalTransferFilterOptions.length
        ? undefined
        : draftInternalTransferFilters
    );
  ...
  const toggleInternalTransferDraft = React.useCallback(
    (value: InternalTransferFilterValue, checked: boolean) => {
      setDraftInternalTransferFilters((current) =>
        checked
          ...
          >
            ...
            <div className="bg-muted/10 flex-1 overflow-y-auto">
              <TabsContent value="filters" className="m-0 focus-visible:ring-0">
                <div className="space-y-6 p-5">
                  <div>
                    <SectionLabel>Internal transfer</SectionLabel>
                    <div className="grid grid-cols-2 gap-2">
                      {[
                        {
                          value: 'Yes' as InternalTransferFilterValue,
                          label: 'Yes',
                          indicator: 'bg-amber-500',
                        },
                        {
                          value: 'No' as InternalTransferFilterValue,
                          label: 'No',
                          indicator: 'bg-emerald-500',
                        },
                      ].map(({ value, label, indicator }) => {
                        const checked =
                          draftInternalTransferFilters.includes(value);
                        return (
                          <button
                            key={value}
                            type="button"
                            onClick={() =>
                              toggleInternalTransferDraft(value, !checked)
                            }
