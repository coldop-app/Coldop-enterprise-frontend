...
function collectNikasiLeafColumnSums(
  ...
  for (const row of rows) {
    const stack: Row<NikasiReportRow>[] = [row];
...
const NIKASI_COLUMN_HEADER_LABELS: Partial<
  Record<keyof NikasiReportRow, string>
> = {
...
function getColumnHeaderLabel(
  column: ReturnType<Table<NikasiReportRow>['getVisibleLeafColumns']>[number]
): string {
  const mappedLabel =
    NIKASI_COLUMN_HEADER_LABELS[
      column.id as keyof typeof NIKASI_COLUMN_HEADER_LABELS
    ];
...
function getExcelBodyRows(
  ...
  const appendRows = (tableRows: Row<NikasiReportRow>[]) => {
