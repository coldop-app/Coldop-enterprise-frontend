...
import { ReportPageNumber, ReportRunHeader } from '../report/pdf/header';
import type {
...
type IncomingDailyBreakdownPdfProps = {
  generatedAt: string;
  coldStorageName: string;
  trendData: DailyMonthlyTrendData;
...
function createDailyRows(
  chartData: DailyMonthlyTrendData['daily']['chartData']
): TrendRow[] {
...
function createMonthlyRows(
  chartData: DailyMonthlyTrendData['monthly']['chartData']
): TrendRow[] {
...
function createTotals(rows: TrendRow[], locations: string[]): TrendPoint[] {
...
function buildInsights(rows: TrendRow[], locations: string[]) {
  ...
  const byLocation = createTotals(rows, locations)
    .map((item) => ({
      ...item,
...
function TrendTable({
  ...
  const totals = createTotals(rows, locations);
  const grandTotal = rows.reduce((sum, row) => sum + row.total, 0);
  const labelColWidth = 18;
  const totalColWidth = 12;
  const locationColWidth = locations.length
    ? (100 - labelColWidth - totalColWidth) / locations.length
    : 70;
        ...
        {locations.map((location) => (
          <Text
            ...
            style={[
              s.tableHeaderText,
              s.tableCellText,
              { width: `${locationColWidth}%`, textAlign: 'right' },
            ]}
          >
            {location}
          </Text>
      ...
      {rows.map((row, index) => (
        ...
        >
          ...
          {locations.map((location) => (
            ...
            >
              {formatNumber(Number(row.perLocation[location] ?? 0))}
            </Text>
          ))}
          <Text
            style={[
...
function InsightsSection({
  ...
  const insights = buildInsights(dailyRows, dailyLocations);
  ...
  return (
    <View style={s.insightsWrap}>
      ...
      <Text style={s.insightsText}>
        This report lists incoming bags by receiving location for each day in
        the range, then totals by location.
      </Text>
      <Text style={s.insightsText}>
        Total bags received in this report: {formatNumber(insights.totalBags)},
        across {insights.locationCount} locations and {insights.totalDays} days
        in the table.
      </Text>
      <Text style={s.insightsMuted}>
        ...
        {formatNumber(Math.round(insights.avgPerDay))}. Among days with receipts
      ...
      <Text style={s.insightsSubHeading}>Bags by location</Text>
      <View style={s.insightsRule} />
      <Text style={s.insightsMuted}>
      ...
      <View style={s.insightsTable}>
        ...
        {insights.byLocation.map((item, index) => (
          <View
            key={`insight-location-${item.key}`}
            style={[
              s.insightsTableRow,
              ...(index === insights.byLocation.length - 1
...
export default function IncomingDailyBreakdownPdf({
  generatedAt,
  coldStorageName,
  trendData,
  dateRangeLabel,
}: IncomingDailyBreakdownPdfProps) {
  const dailySeries = trendData.daily?.chartData ?? [];
  const monthlySeries = trendData.monthly?.chartData ?? [];

  const dailyLocations = dailySeries.map((series) => series.location);
  const monthlyLocations = monthlySeries.map((series) => series.location);
  ...
  return (
    <Document
      title="Incoming Daily Breakdown Report"
      ...
      <Page size="A4" orientation="landscape" style={s.page}>
        ...
        <View style={s.coverBlock}>
          <Image
            ...
            style={s.coverLogo}
          />
          <Text style={s.coverLabel}>Incoming Daily Breakdown Report</Text>
