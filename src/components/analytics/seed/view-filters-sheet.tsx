...
import { CONTRACT_FARMING_COLUMN_OPTIONS } from './contractFarmingColumns';
import type { ContractFarmingTableRow } from './columns';
import {
...
type ViewFiltersSheetProps = {
  table: TanstackTable<ContractFarmingTableRow>;
};

function getColumnLabel(columnId: string): string {
  return (
    CONTRACT_FARMING_COLUMN_OPTIONS.find((c) => c.key === columnId)?.label ??
    columnId
  );
}
...
function SortableRow({ id, label, rightSlot }: SortableRowProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
...
export function ViewFiltersSheet({ table }: ViewFiltersSheetProps) {
  ...
  const availableFilterOptions = useMemo<Record<string, string[]>>(() => {
  ...
  const syncDraftFromTable = () => {
    ...
    const valueFilters: Record<string, string[]> = {};
    filterableColumns.forEach((id) => {
  ...
  const toggleValue = (columnId: string, value: string, checked: boolean) => {
  ...
  const toggleAllValues = (columnId: string) => {
  ...
  const filteredOptionsForColumn = (columnId: string) => {
                        ...
                        <div className="border-border border-t">
                          <div className="border-border relative border-b">
                            ...
                            <input
                              ...
                              placeholder={`Search ${getColumnLabel(id).toLowerCase()}...`}
                    ...
                    >
                      {draftGrouping.map((columnId, index) => (
                        <SortableRow
                          key={columnId}
                          id={columnId}
                          label={`${index + 1}. ${getColumnLabel(columnId)}`}
                          rightSlot={
                            <button
                              type="button"
