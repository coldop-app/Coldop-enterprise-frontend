...
export function ViewFiltersSheet({
  ...
  const allNumericFilterFields = useMemo(() => {
    const dynamicNumericIds = allColumnIds.filter((id) =>
      id.startsWith('farmerSeedBagSize_')
    );
    return [...numericFilterFields, ...dynamicNumericIds];
  }, [allColumnIds]);
  ...
  const advancedFieldValueOptions = useMemo<Record<string, string[]>>(() => {
    ...
    advancedFilterFields.forEach(({ id }) => {
      ...
      options[id] = allNumericFilterFields.includes(id)
    ...
    return options;
  }, [table, advancedFilterFields, allNumericFilterFields]);
    ...
    setDraftLogicFilter((current) =>
      mutateFilterNodeById(current, conditionId, (node) => {
        ...
        return {
          ...node,
          field,
          operator: getDefaultOperatorForField(field, allNumericFilterFields),
          value: '',
        };
      })
    );
      ...
      <div className="space-y-2">
        {group.conditions.length === 0 ? (
          ...
          group.conditions.map((node) => {
            ...
            const isNumeric = allNumericFilterFields.includes(node.field);
            ...
            return (
              ...
              >
                <select
                  value={node.field}
                  onChange={(event) =>
                    setConditionField(node.id, event.target.value)
                  }
