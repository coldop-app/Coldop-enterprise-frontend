...
import { ChevronDown, FileDown, MoreVertical, Settings2 } from 'lucide-react';
...
export interface FarmerProfileGradingGatePassTableProps {
  ...
  farmerName?: string;
  companyName?: string;
  /** Farmer seed rows prepared in parent (main profile page). */
  farmerSeedEntries?: FarmerSeedEntryByStorageLink[];
}
  ...
  >(function FarmerProfileGradingGatePassTable(
    ...
    const handleAccountingReportGenerate = async (selectedIds: Set<string>) => {
      ...
      try {
        ...
        const preparedAccountingPdf = prepareAccountingStockLedgerPdfData({
          snapshot,
          stockLedgerRows: stockLedgerRowsSelected,
          farmerSeedEntries,
        });
        const blob = await pdf(
          <AccountingStockLedgerPdf prepared={preparedAccountingPdf} />
        ).toBlob();
        const url = URL.createObjectURL(blob);
        if (printWindow) {
          printWindow.location.href = url;
            ...
            <div className="flex w-full min-w-0 flex-wrap items-center gap-x-3 gap-y-2">
              <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
                <div className="flex items-center gap-2">
                  <Label
                    htmlFor="farmer-profile-grading-from"
                    className="font-custom text-muted-foreground shrink-0 text-sm font-medium"
                  >
                    From
                  </Label>
                  <DatePicker
                ...
                <div className="flex items-center gap-2">
                  <Label
                    htmlFor="farmer-profile-grading-to"
                    className="font-custom text-muted-foreground shrink-0 text-sm font-medium"
                  >
                    To
                  </Label>
              ...
              >
                ...
                >
                  <ChevronDown
                    ...
                    aria-hidden
                  />
                  {showTableData ? 'Hide table' : 'Show table'}
                </Button>
                <Button
                  variant="default"
                  size="sm"
              ...
              <Table className="border-collapse">
                {showTableData ? (
                  <>
                    ...
                    <TableBody
                      id="farmer-profile-grading-table-body"
                      className="order [&_tr]:border-b [&_tr:last-child]:border-b"
                    >
                      {tableBodyRows}
