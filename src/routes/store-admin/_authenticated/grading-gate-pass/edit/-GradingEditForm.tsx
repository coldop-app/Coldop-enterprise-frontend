...
const ALLOCATION_OPTIONS = ['UNALLOCATED', 'ALLOCATED', 'PARTIAL'] as const;
...
const fullFormSchema = z.object({
  ...
  allocationStatus: z.string().trim().min(1, 'Allocation status is required'),
...
const stepOneSchema = z
  .object({
    ...
    variety: fullFormSchema.shape.variety,
    allocationStatus: fullFormSchema.shape.allocationStatus,
    graderKnownKey: z.string(),
    graderCustom: z.string(),
...
type GradingEditFormProps = { pass: GradingGatePass };
...
export const GradingEditForm = memo(function GradingEditForm({
  ...
  const farmerDisplay = useMemo(() => getFarmerFromPass(pass), [pass]);
  ...
  const allocationDefault = pass.allocationStatus?.trim() || 'UNALLOCATED';

  const allocationSelectOptions = useMemo(() => {
    const uniq = new Set<string>(ALLOCATION_OPTIONS);
    const raw = pass.allocationStatus?.trim();
    if (raw) uniq.add(raw);
    return [...uniq];
  ...
  const form = useForm({
    defaultValues: {
      ...
      variety: pass.variety?.trim() || '',
      allocationStatus: allocationDefault,
      ...
      editGradingGatePass(
        {
          ...
          allocationStatus: value.allocationStatus.trim(),
  ...
  const summaryValues: GradingSummaryFormValues = useMemo(() => {
    ...
    return {
      ...
      incomingLines: buildIncomingLines(pass),
      gradedByLabel: getGradedByLabel(pass),
    };
  }, [form.state.values, pass, farmerDisplay]);
          ...
          <div className="flex flex-1 items-start gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-muted-foreground font-custom text-xs leading-snug">
                Farmer context, voucher date, variety, grader & allocation
              </p>
            </div>
          </div>
              ...
              <Card className="shadow-md">
                ...
                <CardContent className="space-y-3 pt-5">
                  ...
                  <ul className="space-y-2">
                    {(pass.incomingGatePassIds ?? []).map((gp, idx) => (
                  ...
                  <form.Field
                    ...
                    children={(field) => (
                      ...
                      >
                        ...
                        <SearchSelector
                          id="grading-edit-variety"
                          options={POTATO_VARIETIES}
                          placeholder="Select variety"
                          searchPlaceholder="Search variety…"
                          value={field.state.value || undefined}
                          onSelect={(v) => field.handleChange(v ?? '')}
                          emptyMessage="No variety"
                          className="w-full"
                          buttonClassName="font-custom h-11 w-full justify-between rounded-md border border-input shadow-xs"
                          aria-invalid={
                            !!(
                              field.state.meta.errors?.length &&
                              field.state.meta.isTouched
                      ...
                      >
                        ...
                        >
                          <SelectTrigger
                            ...
                            className="font-custom focus-visible:ring-primary h-11 w-full"
                          >
                            <SelectValue placeholder="Select status" />
                          </SelectTrigger>
                          <SelectContent>
                            {allocationSelectOptions.map((opt) => (
                              <SelectItem
                                key={opt}
                                value={opt}
