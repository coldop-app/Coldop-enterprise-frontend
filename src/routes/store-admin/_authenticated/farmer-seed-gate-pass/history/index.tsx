...
import {
  prefetchFarmerSeedEditHistory,
  useGetFarmerSeedEditHistory,
} from '@/services/store-admin/farmer-seed/useGetFarmerSeedEditHistory';
...
export const Route = createFileRoute(
  ...
  loader: () =>
    prefetchFarmerSeedEditHistory({ page: 1, limit: DEFAULT_PAGE_SIZE }),
...
function formatScalarValue(value: unknown): string {
  ...
  if (typeof value === 'string') {
    ...
    // Show ISO-like strings as human readable date-time.
...
function RouteComponent() {
  ...
  useEffect(() => {
    if (!isLoading && !isError && pagination?.hasNextPage) {
      void prefetchFarmerSeedEditHistory({
        page: currentPage + 1,
        limit: DEFAULT_PAGE_SIZE,
      });
    }
        ...
        <div className="space-y-4">
          ...
          {records.map((audit, index) => {
            ...
            const editedBy = item.editedById as
              | Record<string, unknown>
              | undefined;
            const farmerSeed = item.farmerSeedId as
              | Record<string, unknown>
              | undefined;
            const statusValue = farmerSeed?.status ?? item.status;
            const gatePassNumber =
              farmerSeed?.gatePassNo ?? updatedState?.gatePassNo;
              ...
              <Card key={recordKey} className="rounded-xl border shadow-sm">
                ...
                <CardContent className="space-y-4 pt-5">
                  ...
                  <div className="bg-muted/30 rounded-lg border p-3">
                    ...
                    {renderKeyValueBlock({
                      _id: item._id,
                      farmerSeedId: item.farmerSeedId,
            ...
            <Card className="rounded-xl border-dashed">
              <CardContent className="py-10 text-center">
                <p className="font-custom text-base font-medium text-[#333]">
                  {isLoading
                    ? 'Loading farmer seed edit history...'
                    : isError
                      ? 'Failed to load farmer seed edit history'
