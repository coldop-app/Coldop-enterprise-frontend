...
import {
  prefetchStorageEditHistory,
  useGetStorageEditHistory,
} from '@/services/store-admin/storage-gate-pass/useGetStorageEditHistory';
import { StorageEditHistoryCard } from './-storage-edit-history-card';
...
export const Route = createFileRoute(
  ...
  loader: () =>
    prefetchStorageEditHistory({ page: 1, limit: DEFAULT_PAGE_SIZE }),
...
function RouteComponent() {
  ...
  useEffect(() => {
    if (!isLoading && !isError && pagination?.hasNextPage) {
      void prefetchStorageEditHistory({
        page: currentPage + 1,
        limit: DEFAULT_PAGE_SIZE,
      });
    }
    ...
    <main className="mx-auto max-w-4xl p-3 sm:p-4 lg:p-6">
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          ...
          {records.map((audit) => (
            <StorageEditHistoryCard key={audit._id} audit={audit} />
          ))}
            ...
            <Card className="rounded-xl border-dashed">
              <CardContent className="py-12 text-center">
                ...
                <p className="font-custom text-foreground text-sm font-medium">
                  {isLoading
                    ? 'Loading storage edit history...'
                    : isError
                      ? 'Failed to load storage edit history'
